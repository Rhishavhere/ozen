...
export const HubLayout: React.FC = () => {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case "chat":
        return (
          <ChatArea
            key={viewingConversationId || "new"}
            loadConversationId={viewingConversationId}
          />
        );
      case "agents":
        return <AgentsView />;
      ...
      case "features":
        return (
          <div className="w-full h-full flex flex-col items-center justify-center text-gray-400 bg-[#FCFCFD]">
            ...
            <h2 className="text-lg font-bold text-gray-600 mb-1 capitalize">
              {activeTab}
            </h2>
  ...
  return (
    <div
      className="w-screen h-screen flex flex-col bg-white overflow-hidden border border-gray-300"
