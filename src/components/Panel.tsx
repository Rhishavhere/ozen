...
export const Panel: React.FC = () => {
  ...
  const handleSubmit = async (e?: React.FormEvent, triggerImageSearch: boolean = false, overrideQuery?: string) => {
    ...
    // Image search only when user submits with Alt+Enter
    const shouldFetchImages = triggerImageSearch;
        ...
        <AnimatePresence>
          {activeWindow && !isExpanded && messages.length === 0 && (
            <motion.div 
              ...
              exit={{ opacity: 0, y: 5 }}
              className="flex items-center gap-1.5 self-start mb-2 px-3 py-1 bg-white backdrop-blur-md border border-white/40 rounded-full shadow-xs"
            >
              {/* <div className="w-1.5 h-1.5 rounded-full bg-purple-500 animate-pulse" /> */}
              <span className="text-[10px] font-semibold text-black/65 tracking-wide uppercase">
                <span className='text-blue-500 font-medium'>@</span> {activeWindow.owner}
              </span>
            </motion.div>
          )}
        </AnimatePresence>
